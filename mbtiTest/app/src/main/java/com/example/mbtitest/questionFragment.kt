...
class QuestionFragment : Fragment() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        arguments?.let {
            quetionType = it.getInt(ARG_QUESTION_TYPE)
        }
    }
    ...
    override fun onCreateView(
        ...
        title.text = getString(questionTitle[quetionType])
        ...
        for(i in questionTextView.indices){
            questionTextView[i].text = getString(questionTexts[quetionType][i])
            ...
            radioButton1.text = getString(questionAnswers[quetionType][i][0])
            radioButton2.text = getString(questionAnswers[quetionType][i][1])
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        btn_next.setOnClickListener {
            ...
            if (isAllAnswered == true){
                val respones = answerRadioGroup.map {radioGroup ->
                    val firstRadioButton =radioGroup.getChildAt(0) as RadioButton
                    if (firstRadioButton.isChecked) 1 else 2
                }

                (activity as TestActivity)?.questionnaireResults?.addResoponses(respones)
